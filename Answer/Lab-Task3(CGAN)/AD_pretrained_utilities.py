...
class loader(Dataset):
    def __init__(self, dataset, nclasses=2, transform=None, preprocessing=False):
        """
        dataset : list of all filenames (이미지 파일 경로 리스트)
    ...
    def __getitem__(self, index):
        # 이미지 파일과 라벨 파일을 각각 로드
        img_filename = self.dataset[index].replace('_label.npy', '_img.npy')
        label_filename = self.dataset[index]
        
        x_batch = np.load(img_filename)
        y_batch = np.load(label_filename)

        if self.preprocessing:
            x_batch = img_processing(x_batch)  # 필요시 전처리 추가
        if self.transform:
            x_batch = self.transform(x_batch)
        
        # 2개 클래스일 경우 라벨을 0과 1로 수정
...
def predict(net, data_loader, device, return_prob=True):
    ...
    with alive_bar(len(data_loader), bar='classic', spinner='arrow') as bar:
        for _, (x, y) in enumerate(data_loader):
            x = x.to(device).float()  # 데이터 타입을 float32로 변환
